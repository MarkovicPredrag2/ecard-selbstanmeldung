...
module.exports.listen = (ipaddress, cardReader, interval, test = false) => {
	/*
	  ...
	  var requestData = JSON.parse(ginaData);
		if (cardCanBePlugged) {
			...
			if (requestData.patient) {
				//	console.log('Patient wurde gefunden'); -- Uncomment when testing
				//	If so, card can be unplugged now.
				cardCanBePlugged = false;
				//	Invoke callback and return patient data.
				eventEmitter.emit('data', requestData.patient);
				...
				if (requestData.cardinfo.state == 'STATUS_READER_PRESENT') {
					...
					if (test || requestData.cardinfo.type == 'e-card') {
					...
					//	console.log('Karte steckt nicht!');	-- Uncomment when testing
