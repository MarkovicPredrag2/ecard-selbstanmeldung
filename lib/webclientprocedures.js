...
const createUserSession = (user) => {
	return new Promise((resolve, reject) => {
		DB.SQLCommandFeeder(['SELECT session_id FROM sessions'])
			.then((result) => {
				var token = generateRandomString(26);
				for(var n = 0; n < result.length; n++) 
					if(result[n].session_id == token) {
						token = generateRandomString(26);
						n = -1;
					}
...
const checkIfCookieisValid = (cookies, cookieName) => {
	return new Promise((resolve, reject) => {
		DB.SQLCommandFeeder([
				'SELECT benutzer.rolle, sessions.* ' +
				'FROM benutzer INNER JOIN sessions ' +
				'ON benutzer.username = sessions.username'
			])
			.then((session) => {
				var flag = true;
				for(var n = 0; n < session.length; n++)
					if(cookies[cookieName] == session[n].session_id) {
						 ...
						 if(session[n].timestamp >= unixTime(new Date())) {
						 	flag = false;
						 	var sessionVar = session[n];
						 	setTimeStamp(cookies[cookieName], null)
						 		.then((valid) => resolve(sessionVar))
						 		.catch((error) => reject(error));
						 	}
...
const redirectUserToPage = (role, res) => {
	switch(role) {
	...
	return new Promise((resolve, reject) => {
		DB.SQLCommandFeeder(
			[
				...
				'passwort = hashPW(\'' + credentials.password + '\', salt, 1000)'
			]
		)
		.then((user) => {
...
//	Creates a random string token with the specified length
