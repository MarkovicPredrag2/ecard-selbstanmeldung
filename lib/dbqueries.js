...
class SelbstanmeldungsDB extends MySQLWrapper {
      ...
      this.SQLCommandFeeder(
        [
          `INSERT IGNORE INTO w_warteliste VALUES (` +
            ...
            `((SELECT MAX(w_rank) FROM w_warteliste) + 1), ` +
            `UNIX_TIMESTAMP(), ` +
            `${ this.DB.escape(patient.grund) }, ` +
            `null)`,
            //svnr grundid grund anrede vorname nachname alter
          `SELECT p_patient.p_svnr AS 'patient', ` +
          ...
          `FROM (p_patient INNER JOIN w_warteliste ON ${ this.DB.escape(patient.svnr) } = w_patient_p_svnr) ` +
  ...
  callInPatientIntoBehandlungsZimmer(svnr, arzt) {
    return new Promise((resolve, reject) => {
      ...
      this.SQLCommandFeeder(
        [
          `SELECT w_rank as 'rank' FROM w_warteliste WHERE w_patient_p_svnr = ${ this.DB.escape(svnr) }`
        ]
      )
      .then((check) => {
         ...
         if (check[0].rank == 1) {
           this.SQLCommandFeeder(
              [
                ...
                `INSERT INTO beh_behandlung VALUES (null, ${ this.DB.escape(svnr) }, ${ this.DB.escape(arzt) }, 1)`,
           ...
           reject("Patient is not on first place. Can't call him in.")
         }
       })
       .catch((error) => {
         reject(error);
  ...
  dismissPatient(svnr) {
    return new Promise ((result, reject) => {
      ...
      this.SQLCommandFeeder(
        [
          `SELECT w_beh_session as 'behandlung' FROM w_warteliste WHERE w_patient_p_svnr = ${ this.DB.escape(svnr) }`
        ]
      )
      .then((check) => {
        if (check[0].behandlung != null) {
          this.SQLCommandFeeder(
            [
              // IGNORE FOREIGN Check
              `SET foreign_key_checks = 0`,
