...
class SelbstanmeldungsDB extends MySQLWrapper {
  ...
  // TODO: delete from warteliste, update warteliste wenn insert into Behandlung
  // NOTE: Update Log überarbeiten

  // TODO: Erweiterung der "updatePatient" warteliste
  // TODO: Adden der addPatientToWarteliste

  callInPatientIntoBehandlunsZimmer() {
    return new Promise((resolve, reject) => {
      this.SQLCommandFeeder(
        [
          ``
      ...
      this.SQLCommandFeeder(
        [
          `UPDATE p_patient ` +
            ...
            `p_patient.p_plz = ${ this.DB.escape(patient.plz) }, ` +
          ...
          `WHERE ver_v_id = (SELECT v_versicherungsid FROM v_versicherung WHERE v_kuerzel = ${ this.DB.escape(patient.versid) })`
        ]
      )
    });
  }
