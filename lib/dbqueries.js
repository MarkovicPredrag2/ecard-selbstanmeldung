...
class SelbstanmeldungsDB extends MySQLWrapper {
  ...
  lookUpUser(credentials) {
	...
	addPatientIfNotExists(patient) {
      ...
      // TODO: Ändern auf INSERT INTO ON DUPLICATE UPDATE
      for (var i = 0; i < patient.anspruchsdaten.length; i++) {
        addPatientQuery.push(`INSERT INTO p_patient_has_v_versicherung VALUES ('${ this.DB.escape(patient.svnr) }',` +
                              `'${ this.DB.escape(patient.anspruchsdaten[i]) }')` +
                              `ON DUPLICATE KEY UPDATE p_patient.p_patient_has_v_versicherung = '${ this.DB.escape(patient.anspruchsdaten[i]) }'`);
      }

      addPatientQuery.push(`SELECT p_patient.p_svnr as 'svnr', ` +
                                  ...
                                  `p_patient.p_email as 'mail', `  +
                                  `p_patient.telefonnummer as 'telefonnummer' ` +
                                  ...
                                  `WHERE p_svnr = '${ this.DB.escape(patient.svnr) }'`);

			this.SQLCommandFeeder(addPatientQuery)
				.then((patientresultat) => {
          var patientenDaten = patientresultat[0];
          this.SQLCommandFeeder(
            [
          ...
          .then((versicherungsresultat) => {
            patientenDaten.anspruchsdaten = versicherungsresultat;
            resolve(patientenDaten);
          })
          .catch((error) => {
            reject(error);
          })
  ...
  updatePatient(patient) {
    return new Promise((resolve, reject) => {
      ...
      this.SQLCommandFeeder(
        [
          `UPDATE p_patient` +
          `SET p_patient.p_titel = '${ this.DB.escape(patient.titel) }', ` +
          `SET p_patient.p_gender =  '${ this.DB.escape(patient.geschlecht) }', ` +
          `SET p_patient.p_nachname = '${ this.DB.escape(patient.nachname) }', ` +
          `SET p_patient.p_vorname = '${ this.DB.escape(patient.vorname) }', ` +
          `SET p_patient.p_gebdat = '${ this.DB.escape(patient.geburtsdatum) }', ` +
          `SET p_patient.p_hausnummer = '${ this.DB.escape(patient.hausnummer) }', ` +
          `SET p_patient.p_ort = '${ this.DB.escape(patient.ort) }', ` +
          `SET p_patient.p_plz = '${ this.DB.escape(patient.plz) }', ` +
          `SET p_patient.p_postfachnummer = '${ this.DB.escape(patient.postfachnummer) }', ` +
          `SET p_patient.p_postfachtext = '${ this.DB.escape(patient.postfachtext) }', `+
          `SET p_patient.p_staatcode = '${ this.DB.escape(patient.staatcode) }', ` +
          `SET p_patient.p_stocktuernummer = '${ this.DB.escape(patient.stocktuernummer) }', ` +
          `SET p_patient.p_strasse = '${ this.DB.escape(patient.strasse) }', ` +
          `SET p_patient.p_email = '${ this.DB.escape(patient.email) }', ` +
          `SET p_patient.p_telefonnummer = '${ this.DB.escape(patient.telefonnummer) }' ` +
          `WHERE p_patient.p_svnr = '${ this.DB.escape(patient.svnr) }'`
          `UPDATE ver_versicherungen` +
          `SET vers_v_id = '${ this.DB.escape(patient.versid) }'`
        ]
      )
    });
  }
